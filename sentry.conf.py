...
def file_env(name):
    ...
    if env(file_name) and env(name):
        raise ValueError("both %s and %s are set" % (name, file_name))


    if env(file_name):
        with open(env(file_name)) as f:
            return f.read().strip()
    else:
        return env(name)
...
if email:
    ...
    SENTRY_OPTIONS['mail.password'] = file_env('SENTRY_EMAIL_PASSWORD') or ''
...
# being invalidated.
secret_key = file_env('SENTRY_SECRET_KEY')
if not secret_key:
...
if 'OPENID_CLIENT_ID' in os.environ:
